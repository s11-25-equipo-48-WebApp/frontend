...
export default function AnalyticsPage() {
  ...
  const [viewDetailsModal, setViewDetailsModal] = useState<{ id: string | null; }>({ id: null });
  const [dateRangeLabel, setDateRangeLabel] = useState('Sin filtro');
  const [eventType, setEventType] = useState<'view' | 'submission' | 'approval' | 'rejection' | 'consent_given' | 'consent_revoked' | ''>('');
  ...
  const formatDateRangeLabel = (start: string, end: string) => {
    ...
    return `${startDate.toLocaleDateString('es-ES', { day: 'numeric', month: 'short', year: '2-digit' })} - ${endDate.toLocaleDateString('es-ES', { day: 'numeric', month: 'short', year: '2-digit' })}`;
  };
  ...
  const handleDateRangeSelect = (range: { start_date: string, end_date: string; }) => {
    setDateRange(range);
  ...
  const eventTypeOptions = [
    ...
    { value: 'consent_given', label: 'Consentimiento otorgado' },
    { value: 'consent_revoked', label: 'Consentimiento revocado' },
  ];

  const { data: analyticsData, isPending, error } = useQuery({
    queryKey: ['analyticsData', dateRange, eventType, searchQuery, currentPage, limit],
    queryFn: async () => {
  ...
  const stats = [
    ...
    { label: 'Tasa de aprobación', value: `${percentOfApproval.toFixed(2)}%`, color: 'from-orange-400 to-orange-600' },
    { label: 'Tasa de consentimiento', value: `${percentOfConsent.toFixed(2)}%`, color: 'from-purple-400 to-purple-600' },
    { label: 'Visualizaciones', value: TestimonialsViews, color: 'from-slate-700 to-slate-900' },
  ];
  return (
    <div className="">
      <div className="max-w-7xl mx-auto gap-6 flex flex-col ">
        ...
        <div className="flex flex-col ">
          <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="absolute top-full left-0 mt-2 w-56 bg-card border border-foreground/10 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
                {dateRangeOptions.map((option, index) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 hover:bg-background transition-colors ${dateRange.start_date === option.value.start_date && dateRange.end_date === option.value.end_date
            ...
            {/* Event Type */}
            <div className="flex items-center gap-2 px-4 py-2 bg-card border border-foreground/10 rounded-lg hover:border-winered/50 cursor-pointer transition-colors group relative">
              <span className="text-foreground">{eventType ? eventTypeOptions.find(opt => opt.value === eventType)?.label : 'Tipo de evento'}</span>
              <ChevronDown className="w-4 h-4 text-foreground/60" />
              ...
              <div className="absolute top-full left-0 mt-2 w-48 bg-card border border-foreground/10 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
                <button
                  ...
                  className={`w-full text-left px-4 py-3 hover:bg-background transition-colors first:rounded-t-lg ${eventType === '' ? 'text-winered font-semibold' : 'text-foreground'
                    }`}
                >
                  Todos los eventos
                </button>
                {eventTypeOptions.map((option) => (
                  <button
                    key={option.value}
                    onClick={() => setEventType(option.value as string as typeof eventType)}
                    className={`w-full text-left px-4 py-3 hover:bg-background transition-colors ${eventType === option.value ? 'text-winered font-semibold' : 'text-foreground'
                      }`}
                  >
                    {option.label}
                  </button>
              ...
              <div className="absolute top-full right-0 mt-2 w-32 bg-card border border-foreground/10 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
                {[5, 10, 20, 50].map((value) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 hover:bg-background transition-colors ${limit === value ? 'text-winered font-semibold' : 'text-foreground'
          ...
          isPending && !error ? (
            ...
            <div className="bg-card rounded-lg border border-foreground/10 overflow-hidden">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-foreground/10">
                      <th className="text-left px-6 py-4 font-semibold text-foreground">ID del evento</th>
                      <th className="text-left px-6 py-4 font-semibold text-foreground">Testimonio</th>
                      <th className="text-left px-6 py-4 font-semibold text-foreground">Tipo de evento</th>
                      <th className="text-left px-6 py-4 font-semibold text-foreground">Fecha/Hora</th>
                      <th></th>
                    </tr>
                  </thead>
                  <tbody>
                    {!error && analyticsData?.data && analyticsData.data.length > 0 ? analyticsData.data.map((item: analyticsEvent, index: number) => (
                      <tr key={index} className="border-b border-foreground/5 hover:bg-background/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="px-4 py-2 border border-foreground/10 rounded-lg text-foreground/70 inline-block">
                            {new Date(item.fecha_hora).toLocaleString('es-ES', { hour12: false, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }).replace(',', ' ')}
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <Button
                            variant="ghost"
                            onClick={() => setViewDetailsModal({ id: item.id })}
              ...
              <div className="flex items-center justify-center gap-4 px-6 py-6 border-t border-foreground/10">
                ...
                <div className="flex items-center gap-2">
                  {analyticsData?.meta && (
                    <>
                      ...
                      {Array.from({ length: analyticsData.meta.totalPages }, (_, i) => i + 1).map((page) => {
                ...
                <button
                  onClick={() => setCurrentPage(Math.min(analyticsData?.meta?.totalPages || 1, currentPage + 1))}
                  disabled={currentPage === (analyticsData?.meta?.totalPages || 1)}
                  className="p-2 hover:bg-background rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      {/* View Details Modal */}
      {viewDetailsModal.id && <EventModal id={viewDetailsModal.id} onClose={() => setViewDetailsModal({ id: null })} />}
    </div>
  );
};
